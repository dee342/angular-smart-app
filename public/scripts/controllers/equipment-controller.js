...
angular.module('OpsBoard')
  .controller('EquipmentCtrl', function ($rootScope, $scope, $modal, $log, OpsBoardRepository, 
    ...
    angular.element('.snap-toggle').on('click', function(e) {
      e.preventDefault();
      var toggle = angular.element(this);
      if(/left/.test(toggle.html())) {
        angular.element('#nav').find('div.text').hide(); //fadeOut(500);
        angular.element('#nav').css({width: '43px', 'min-width': '43px'});
        toggle.html('<i class="fa fa-arrow-circle-right fa-lg"></i>');
      }
      else {
        angular.element('#nav').css({width: '193px', 'min-width': '193px'}).find('div.text').fadeIn(500);
        toggle.html('<i class="fa fa-arrow-circle-left fa-lg"></i>');
      }
      return false;
    });

      angular.element('.snap-toggle1').on('click', function(e) {
        e.preventDefault();
        var toggle = angular.element(this);
        if(/left/.test(toggle.html())) {
          angular.element('#nav').hide().fadeOut(500);
          toggle.html('<i class="fa fa-arrow-circle-right fa-lg"></i>');
        }
        else {
          angular.element('#nav').show().fadeIn(500);
          toggle.html('<i class="fa fa-arrow-circle-left fa-lg"></i>');
        }
        return false;
      });
