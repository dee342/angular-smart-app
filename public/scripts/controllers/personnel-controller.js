...
angular.module('OpsBoard').controller(
    ...
    function ($scope, $rootScope, OpsBoardRepository, states, groups, titleHierarchy, titles, $timeout, OpsBoardInterval, PersonnelHelperService, $filter, BoardValueService, BoardHelperService) {
      ...
      $scope.showPersonDetails = function(event, personId) {
        ...
        var cachedLocations = $scope.tasks ? $scope.tasks.locations : {},
        ...
        var unavailableGroupFilter = function (mdaFiltered, actualMdaFiltered) {
          ...
          /* the benefits of unit-test: find the bug:
           *  curry function and bind() not work. the following work fine:
           */
          var codeFilterPartial = function(code) {
              return $filter('personnelCodeFilter')($scope.persons, $scope.filterType, mdaFiltered, $scope.board.realDate, code);
          };
          //var codeFilterPartial = $filter('personnelCodeFilter').bind($scope.persons, $scope.filterType, mdaFiltered, $scope.board.realDate);
        ...
        $scope.unavailableGroupFilter = unavailableGroupFilter;
        ...
        var cachedShiftData = $scope.shiftData ? $scope.shiftData.toJSON() : {};
        ...
        angular.element(document).find('#search-person-clear').on('click', function(e) {
            e.preventDefault();
            angular.element(this).siblings('input').val('').focus();
            $scope.person_search.formattedName = '';
            $scope.$digest();
        });
