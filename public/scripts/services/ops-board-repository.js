...
angular.module('OpsBoard').service(
    ...
    function ($rootScope, $resource, states, $filter, $timeout, $window, $interval, user, titles, 
        ...
        EquipmentHelperService, PersonnelHelperService, ReportsService, OpsBoardInterval, $http, $q, $log, $sce,
        BoardValueService, CategoryDataService) {

      $log.info('data loaded %s', $window.performance && $window.performance.now());
      ...
      *************************************************************************************************/
      ...
      var equipmentLocations = BoardValueService.equipmentLocations;
          ...
          if (equipmentLocations && equipmentLocations.length > 1){
      ...
      function init (res) {
        ...
        $log.info('init %s', $window.performance && $window.performance.now());
        ...
        $log.info('init processed (async) %s', $window.performance && $window.performance.now());
